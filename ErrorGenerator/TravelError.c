...
void DEFINED_LOCATION_FirstLocation(){
    ...
    printf("%s%d%s", "Hello Summoner with", PLAYER_STRUCT.health, "\nWelcome to FIRST Location");
    COMPILER_TOOL_WAIT_FOR_INPUT();

    printf("\n1. Second Location"); 

    int USER_CHOICE_INPUT_PNJmVvw8 = COMPILER_TOOL_GET_INPUT(1);

  if (USER_CHOICE_INPUT_PNJmVvw8 == 1){
...
void DEFINED_LOCATION_SecondLocation(){
    ...
    int USER_CHOICE_INPUT_NkKkdowF = COMPILER_TOOL_GET_INPUT(2);

  if (USER_CHOICE_INPUT_NkKkdowF == 1){

DEFINED_LOCATION_FirstLocation();
  }

  if (USER_CHOICE_INPUT_NkKkdowF == 2){
